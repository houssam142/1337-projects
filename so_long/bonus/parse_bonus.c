...
/*   Updated: 2025/03/14 00:58:54 by hounejja         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!line_all_ones_b(map[0], '1')
		|| !line_all_ones_b(map[tab_length_b(map) - 1],
			'1'))
	{
		write(2, "Error\n", 6);
...
char	**check_map(char *map, t_game *w)
{
	...
	ft_join_and_free_b(str, characters + ft_strlen1_b(*str));
	free(characters);
	return (str);
}
